...
| *(no repo, branch do agente)* `docs/corpus/justice-1952-texto-integral-colagem-dono.md` | ✅ **RECEBIDO e auditado (03/09)** — texto INTEGRAL do capítulo colado pelo Dono (Introdução + Outline 41/41 + References completas + Cross-References + Additional Readings); auditoria: 18/18 refs 8c(1) == extração rodada 1; divergência 1952×benchmark constatada na fonte; p.858 resolvida. RAW de conferência; **selo canônico = PDF na `corpus` (upload pendente)** | 132.048 car. (raw) | n/a (texto) | n/a | Mesmo conteúdo declarado do PDF: Syntopicon Vol. 2, JUSTICE, pp. 850–879 (1952) | 03/09/2026, colagem direta do Dono |
| *(no repo, branch do agente)* `data/justice-1952-full.ts` | ✅ **INGERIDO (04/09)** — dataset AUTO-GERADO pelo `scripts/ingest-justice-1952.ts` a partir da colagem do Dono (parser RAW FIRST; travas de estrutura; regenerável com `npx tsx scripts/ingest-justice-1952.ts`): 41 nós · 826 refs (795+31) · 10 remissões · 0 warnings · locatorRaw intocado. Selo canônico herda o status do PDF (aguarda upload) | 483.248 B | n/a (texto gerado) | n/a | Derivado 1:1 da colagem `justice-1952-texto-integral-colagem-dono.md` | 04/09/2026, via Arena |
